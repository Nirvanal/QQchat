...
public class UserClientService {
    private User u=new User();//因为我们可能在其他地方使用user信息，因此作出成员属性
    //因为Socket在其他地方可能使用，因此作出属性
    private Socket socket;

    //根据userId 和pwd到服务器验证用户是否合法
    ...
    //创建User对象
        ...
        try {
            ...
            if(ms.getMesType().equals(MessageType.MESSAGE_LOGIN_SUCCEED)){//登录OK
                ...
                //这里为了后面客户端的扩展，我们将线程放入到集合中管理

                ManageClientConnectServerThread.addClientConnectServerThread(userId,clientConnectServerThread);

                b=true;
            }else{
                //如果登陆失败,我们就不能使用和服务器通信的线程，关闭socket
                socket.close();
